...
unsigned int Random() {
	...
	int x;
	for (int i = 0; i < 500; i++) {
		x = rand();//trong vong lap
	}
	return x;
}

unsigned int Random_MADG(TreeTDG t) {
	unsigned int temp;
	do {
		temp = Random();
...
void Create_ArrMADG(TreeTDG t, int* arr) {
	if (t == NULL)
		return;
	Create_ArrMADG(t->leftTDG, arr); // tham trai
	arr[index++] = t->info.maThe;
	Create_ArrMADG(t->rightTDG, arr);// tham phai
}

void Create_ArrTenHo(TreeTDG t, TEN_HO* arr) {
	if (t == NULL)
		return;
	Create_ArrTenHo(t->leftTDG, arr); // 
	arr[index].hoten = Get_TenHO(t->info);
	arr[index++].MADG = t->info.maThe;
	Create_ArrTenHo(t->rightTDG, arr);//
}
...
void Update_DG(TreeTDG &t, TDG &dg, bool isEdited) {
	...
	CreateBox(X_NOTIFY, Y_NOTIFY, "Luu Y:  ", 59);
